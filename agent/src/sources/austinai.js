  ...
  try {
    ...
    for (const eventUrl of eventUrls) {
      try {
        ...
        // Extract event details using Tribe Events classes
        ...
        const venue = $('.tribe-venue').text().trim() ||
                      $('.tribe-events-venue-details .tribe-venue-name').text().trim();
        const address = $('.tribe-address').text().trim();
        const description = $('.tribe-events-single-event-description').text().trim().substring(0, 500);
