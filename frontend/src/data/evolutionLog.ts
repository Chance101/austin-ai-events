...
export const stewardshipLog: StewardshipEntry[] = [
  {
    id: 'dedup-by-source-event-id',
    date: '2026-04-25',
    title: 'Application-Layer Dedup by (source, source_event_id) — Catch URL-Drift Misses',
    problem: 'Investigation after fixing the upsert counter found a structural fragility in the dedup pipeline. URL-hash dedup at agent/src/index.js:599 was the only fast-path check before fuzzy/Claude dedup, but the URL hash can drift across runs while the event\'s identity is unchanged — anytime a scraper update changes which external link it picks for an event we already have. The 2026-04-25 run surfaced exactly this: the outer-loop\'s autonomous repair commit on 2026-04-20 (5bd2d8e Fix Capital Factory scraper last-link-wins URL bug) tightened URL extraction; on the next run, "Capital Factory: Health Supernova" had a different URL hash than its 2026-04-19 row, slipped past URL-hash and fuzzy dedup, paid for a Claude classify call, and silently became an UPDATE at the DB level via the (source, source_event_id) unique constraint. The 30-day overcount audit found 4 such phantom adds across 3 days. Source-ID stability audit confirmed all 8 config sources produce stable, content-derived identifiers (luma slugs, meetup numeric IDs, AICamp event IDs, capital-factory date+title slugs) — no timestamps or run-time variable elements. ~85% of stored row volume has a stable source_event_id (only web-search routinely lacks one).',
    action: 'New pure helper getEventIdKey(event) in agent/src/utils/dedup.js mirrors the DB unique constraint at the application layer — returns `${source}|${source_event_id}` or null when either is missing. In agent/src/index.js, alongside the existingHashes Set, build an existingByIdKey Map keyed on getEventIdKey(). The fast-path dedup check now matches on URL hash OR ID key — whichever finds a row first wins, and a single matchSignal field ("url" | "source_event_id" | "url+id") flows into both the console log and the decision log so the monitor can see which signal caught a duplicate. The post-insert bookkeeping at line 805 also seeds existingByIdKey so newly-added events are dedup\'d within the same run. 7 new test cases in dedup.test.js cover the helper behavior plus a named regression test reproducing the exact Capital Factory URL-drift scenario.',
    result: 'URL drift between scrapes no longer bypasses fast-path dedup for any event whose source provides a stable ID — which is every config source we currently scrape. The wasted Claude classify call (~$0.001) per missed dedup goes away. Telemetry stays accurate even when scrapers churn URL extraction logic. The fix mirrors the DB constraint at the application layer rather than relying on Postgres ON CONFLICT to silently turn inserts into updates. Combined with the 2026-04-25 upsert counter accuracy fix, the events_added counter now reflects ground truth from two independent angles: app-layer dedup catches duplicates before the upsert call, AND the upsert callsite distinguishes insert from update via created_at comparison if anything still slips through. 155 tests pass (148 existing + 7 new).',
    category: 'foundation',
  },
  {
    id: 'upsert-counter-accuracy',
    date: '2026-04-25',
    title: 'Upsert Counter Accuracy — Stop Counting Updates as New Events',
    problem: 'Reviewing the 2026-04-25 run, the agent reported events_added=1 (Capital Factory: Health Supernova) but the database showed that row was actually created on 2026-04-19 — only its updated_at field jumped to today. The pipeline\'s upsert call to Supabase used onConflict on (source, source_event_id), and Postgres silently turned what looked like an INSERT into an UPDATE because the unique constraint matched a pre-existing row. The Supabase JS client doesn\'t expose whether the operation was an insert or update — it just returns the row. The pipeline blindly incremented runStats.eventsAdded, logged "✅ Added", and tagged the decision log entry as accepted. The monitor then read events_added from agent_runs and reported "added 2 events from Capital Factory" in its summary — also wrong. The 2026-02-10 phantom-accepted fix had widened the existing-events fetch window to catch past-event re-scrapes, but that didn\'t help here: Health Supernova\'s start_time was 2026-05-14 (future), it WAS in the dedup set, and the URL-hash dedup still missed it because the scraped URL drifted slightly between the 2026-04-19 and 2026-04-25 runs. So a different miss mode reached the same broken counter.',
    action: 'Single-callsite fix at agent/src/index.js around the upsertEvent call (~line 766). After the upsert returns, parse result.created_at into a timestamp and compare it against runStats.startTime with a 5-second buffer for clock drift between the GitHub Actions runner and Supabase. If created_at >= startTime - 5s, the row was freshly inserted on this run — increment eventsAdded and log "✅ Added". Otherwise the upsert touched a pre-existing row — increment eventsUpdated and log "🔄 Refreshed". The decisionLog outcome stays as "accepted" for backwards compatibility (the in-memory aggregator at decisionLog.js:51 and the frontend DecisionLog.tsx component both depend on that string), but a new details.persisted field carries "inserted" | "updated" for monitor diagnostics. No changes to upsertEvent itself — keep the supabase wrapper pure. No new DB queries — created_at is already returned by the existing .select().single() path.',
    result: 'eventsAdded now reflects only genuinely new rows. eventsUpdated picks up the second update path (the one where URL-hash dedup missed but the (source, source_event_id) constraint caught it at the DB layer). The monitor\'s "added N events" summary will stop being inflated by upsert-touches. The fix is reversible, single-callsite, no migration. The deeper underlying issue — that URL-hash dedup misses rows whose URL drifts but whose source_event_id is stable, wasting a Claude classification call per miss — is intentionally NOT fixed here. That\'s a separate investigation tracked as a follow-up: build an existingByIdKey Map alongside existingHashes in the early dedup loop, keyed on `${source}|${source_event_id}` (skipping nulls for web-search rows), so ID-based dedup catches what URL-hash dedup misses. All 148 tests pass.',
    category: 'foundation',
  },
  {
    id: 'classifier-enum-sanitizer',
    date: '2026-04-25',
    title: 'Classifier Enum Sanitizer — Defense at the LLM-to-DB Boundary',
    problem: 'The Haiku classifier in agent/src/utils/claude.js produces an audienceType array that the pipeline writes straight into the events table\'s audience_type[] column — a Postgres ENUM with exactly five valid values: developers, business, researchers, general, students. The prompt mentioned the valid values inline as a comment-style hint, but the pipeline never validated Haiku\'s output before the upsert. On 2026-04-12 the monitor flagged a real failure: Haiku returned ["entrepreneurs"], the upsert exploded with "invalid input value for enum audience_type: entrepreneurs", and the event was lost. The action item sat in the human-review queue for 13 days because the monitor pointed at non-existent files (agent/src/classify.js, agent/src/events.js) and the outer loop\'s scope-gate couldn\'t auto-fix what it couldn\'t find. Worse: this is a class of bug, not a single value — any future drift (founders, executives, engineers) would break the same way, and technicalLevel had identical risk surface even though it hadn\'t blown up yet.',
    action: 'Added defense-in-depth at the LLM-to-DB boundary in agent/src/utils/claude.js. Two layers: (1) Module-level constants VALID_AUDIENCE_TYPES and VALID_TECHNICAL_LEVELS mirror the Postgres enum values with a comment pointing at supabase/schema.sql as the canonical source — single source of truth in the codebase, mirrored once. (2) New pure function sanitizeClassification(parsed) runs on every parsed Haiku response before it returns from classifyEvent. It intersects audienceType against VALID_AUDIENCE_TYPES (drops invalid values, falls back to ["general"] if nothing valid remains), validates technicalLevel against VALID_TECHNICAL_LEVELS (coerces invalid values to "all-levels"), normalizes isFree to boolean-or-null, and coerces non-string summary/reasoning to null. Drops are logged via console.warn so the monitor can see drift over time without the upsert dying. (3) Tightened the prompt language from comment-style hints to hard constraints — "MUST contain only values from this exact list — do not invent categories" instead of "array from: ...". The whitelist makes rare drift safe; the prompt change reduces the rate. (4) New agent/src/utils/claude.test.js with 18 cases including a regression test named after action item 257340c2 that feeds the exact "entrepreneurs" failure mode and asserts the output is enum-safe.',
    result: 'sanitizeClassification is a pure function that intercepts every Haiku classification before the upsert. The exact failure mode from 2026-04-12 (audienceType: ["entrepreneurs"]) now produces audienceType: ["general"] with a console.warn flagging the drop — no exception, event still flows through, signal preserved for the monitor. technicalLevel gets the same protection prophylactically. All 148 tests pass (130 existing + 18 new). The fix doesn\'t require a database migration, doesn\'t touch the upsert callsite in index.js (single source of truth — classifier returns clean data), and matches the existing validateEvent fallback pattern at the LLM boundary. Action item 257340c2 marked resolved. The deeper lesson: when an outer-loop proposal points at non-existent files, the human review step is doing real work — the monitor can hallucinate file paths but the proposal itself was correct, and the right place to fix it was claude.js where classifyEvent actually lives.',
    category: 'foundation',
  },
  {
    id: 'aicamp-detail-page-extraction-fix',
    date: '2026-04-15',
    title: 'AICamp Detail Page Extraction — Title, Venue, Description All Silently Dropping',
    problem: 'User flagged a duplicate pair in the calendar — "AI Lab (Austin)" May 6 appearing as two rows. AICamp was the canonical source (the user confirmed: AICamp coordinator drives participation through multiple Meetup groups) but its row had the WRONG title ("Giving AI Agents Real Memory" vs current page title), WRONG time (17:30 UTC vs correct 22:30 UTC), NULL venue_name, generic "Austin, TX" address, and no description — while the Meetup-sourced duplicate had the correct time, the specific venue "Capital Factory", and a better description. User noted: "All of the things that [the duplicate] does better are available in the AICamp webpage, so not sure why it missed it."',
    action: 'Root-cause investigation on the live AICamp event page surfaced three compounding bugs in agent/src/sources/aicamp.js fetchEventDetail(): (1) Title selector looked at h1/h2 but AICamp detail pages use h4 — selector returned empty string, the "if (title)" null-guard fired, and fetchEventDetail returned null for EVERY AICamp event. (2) When fetchEventDetail returned null, the caller fell back to listing-only data which had no description, no venue, and hardcoded address="Austin, TX". (3) Even if title had worked, the fallback description used <meta name="description"> which is AICamp\'s SITE-WIDE generic blurb, not the event-specific content. Rewrite of fetchEventDetail: title via meta[property="og:title"] (reliable primary) with h4 / h1 / h2 fallbacks; description by gathering the first substantive <p> tags inside <div class="left-contents"> while filtering out metadata sections (Venue, Speaker, Agenda, Prerequisite); venue/address by targeting the explicit <p>Venue:...</p> block and splitting the address line into venue_name + street address; removed the hardcoded "Austin, TX" address. Also fixed the duplicate row: updated the canonical aicamp row via SQL with the now-correct scraper output (title, description, start/end time, venue, address, image) and soft-deleted the Meetup duplicate via deleted_at + merged_into_id pointing at the canonical row.',
    result: 'AICamp scraper now returns complete event data — verified against the live page: title "AI Lab (Austin) - Building AI Agents with Memory", start_time 22:30 UTC (5:30 PM CDT), end_time 01:30 UTC next day, venue_name "Capital Factory", address "701 Brazos St, Austin, TX 78701", full event description. Also surfaced a systemic improvement: the previous version was silently failing on ALL AICamp detail pages and returning only 1 upcoming event from a 44-event listing — after the fix, 30 upcoming events extract correctly (the others were false-past events because the listing-page-only fallback had the wrong timezone and was filtering them out). The canonical AI Lab row in the DB now has all correct fields, and the Meetup duplicate is soft-deleted with a merge trail. 131/131 tests still passing.',
